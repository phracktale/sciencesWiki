...
docker compose --env-file .env.prod exec -T api php -d memory_limit=1024M bin/console harvester:embed --limit=5000 >> ~/scienceswiki/embed-cron.log 2>&1
docker compose --env-file .env.prod exec -T api php -d memory_limit=1024M bin/console harvester:suggest-placement --limit=5000 >> ~/scienceswiki/embed-cron.log 2>&1
