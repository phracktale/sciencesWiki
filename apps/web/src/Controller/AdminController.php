...
final class AdminController extends AbstractController
{
    ...
    public function settingsAi(Request $request): Response
    {
        return $this->saveSettingsPage($request, 'admin_settings_ai', 'admin/settings_ai.html.twig', static fn (Request $r): array => [
            ...
            'ai.extractor_model' => (string) $r->request->get('extractor_model'),
        ], withModels: true);
    }
    ...
    #[Route('/admin/harvest/refresh-credit', name: 'admin_harvest_refresh_credit', methods: ['POST'])]
    public function harvestRefreshCredit(Request $request): JsonResponse
    {
        if (!$this->admin->isLogged()) {
            return new JsonResponse(['error' => 'Non authentifié.'], 401);
        }
        ...
        $result = $this->admin->refreshHarvestCredit();
